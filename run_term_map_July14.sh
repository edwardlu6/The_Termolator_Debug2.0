...
# Ensure TERMOLATOR directory and language are set
TERMOLATOR_DIR="${5:-$(dirname "$0")}"
LANG_ACRO="$6"
...
python3 "$TERMOLATOR_DIR/make_io_file.py" "$1" "$3.internal_terms_list" .terms

if [ "$LANG_ACRO" = "zh" ]; then
   python3 "$TERMOLATOR_DIR/summary/term_generator.py" "$3"
fi

if [ "$LANG_ACRO" = "fr" ]; then
   python3 "$TERMOLATOR_DIR/summary/term_generator.py" "$3"
fi

python3 "$TERMOLATOR_DIR/run_term_map.py" "$2" "$3.internal_terms_list" "$3.term_instance_map" "$4"
