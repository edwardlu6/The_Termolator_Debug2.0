...
# The script no longer accepts explicit TERMOLATOR or TreeTagger paths.
# If the 9th or 10th argument is missing, fall back to the directory in
# which this script resides and its bundled TreeTagger.  Otherwise, stop
# with a helpful message so callers know to omit these parameters.
if [ -z "$9" ] || [ -z "$10" ]; then
    termolator_location="$(cd "$(dirname "$0")" && pwd)"
    treetagger_location="$termolator_location/TreeTaggerLinux"
else
    echo "This script determines TERMOLATOR and TreeTagger locations automatically."
    echo "Please run without arguments 9 and 10." >&2
    exit 1
fi
